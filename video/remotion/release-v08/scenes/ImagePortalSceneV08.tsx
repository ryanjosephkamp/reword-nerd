...
const queue = [
  { name: "pyramid.webp", src: staticFile("image/orange-pyramid.webp") },
  { name: "workbench.webp", src: staticFile("media/demo/overview-poster.webp") },
  { name: "package.webp", src: staticFile("media/demo/package-poster.webp") },
] as const;

export function ImagePortalSceneV08() {
  const frame = useCurrentFrame();
  const focusedSource = queue[0];
  ...
  return <ReleaseV08Shell durationInFrames={144} eyebrow="IMAGE WORKBENCH" scene="02">
    <div style={{ marginTop: 24, height: 520, border: `1px solid ${colors.orange}`, background: colors.surface, boxShadow: "0 22px 56px rgba(0,0,0,.35)", ...reveal(4) }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "258px minmax(0,1fr) 300px", height: 468 }}>
        <section style={{ padding: 16, borderRight: `1px solid ${colors.border}`, ...reveal(18) }}><div style={{ color: colors.orange, fontSize: 12, letterSpacing: 1.6 }}>IMAGE SET</div><h2 style={{ margin: "7px 0 15px", fontSize: 26 }}>IMAGES</h2>{queue.map((source, index) => <div key={source.name} style={{ display: "grid", gridTemplateColumns: "45px 1fr", gap: 10, padding: 10, marginTop: 9, border: `1px solid ${index === 0 ? colors.orange : colors.border}`, background: index === 0 ? "#1b150e" : colors.canvas, ...reveal(26 + index * 10) }}><Img src={source.src} style={{ width: 45, height: 36, objectFit: "cover" }} /><div><strong style={{ display: "block", fontSize: 11 }}>{source.name}</strong><span style={{ color: colors.muted, fontSize: 9 }}>INCLUDED · OCR OFF</span></div></div>)}</section>
        <section style={{ padding: 16, borderRight: `1px solid ${colors.border}`, ...reveal(30) }}><div style={{ color: colors.orange, fontSize: 12, letterSpacing: 1.6 }}>FOCUSED SOURCE</div><h2 style={{ margin: "7px 0 14px", fontSize: 26 }}>PREVIEW</h2><div style={{ height: 295, display: "grid", placeItems: "center", overflow: "hidden", border: `1px solid ${colors.border}`, background: colors.canvas }}><Img src={focusedSource.src} style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div><p style={{ margin: "13px 0 0", color: colors.muted, fontSize: 12 }}>{focusedSource.name} · exact local source bytes</p></section>
