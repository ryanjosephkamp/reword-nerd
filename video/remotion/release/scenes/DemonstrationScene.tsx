...
export function DemonstrationScene({ demonstrationLabel, feedback }: ReleaseUpdateProps) {
      ...
      <div style={{ padding: "26px 30px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 278px", gap: 20, marginTop: 26 }}>
          <div style={{ display: "grid", gap: 12 }}>
            {["reword-nerd v0.7", "Road to v0.6"].map((title, index) => <div key={title} style={{ padding: "19px 20px", border: `1px solid ${index === 0 ? colors.mint : colors.border}`, background: index === 0 ? "#10201e" : colors.raised, ...reveal(22 + index * 18) }}><div style={{ color: colors.muted, fontSize: 14 }}>{index === 0 ? "CURRENT RELEASE" : "RETROSPECTIVE"}</div><div style={{ marginTop: 8, fontSize: 25, color: index === 0 ? colors.mint : colors.text }}>{title}</div></div>)}
          </div>
          <div style={{ display: "grid", gap: 12 }}>
